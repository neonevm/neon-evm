...
def test_create_same_accounts(
    ...
    create_same_accounts_internal(
        user_account, second_account, evm_loader, operator_keypair, treasury_pool, 2, ACCOUNT_INFO_LAYOUT.sizeof()
    )
    create_same_accounts_internal(
        user_account, second_account, evm_loader, operator_keypair, treasury_pool, 3, MAX_PERMITTED_DATA_INCREASE
    )
    create_same_accounts_internal(
        user_account, second_account, evm_loader, operator_keypair, treasury_pool, 4, MAX_PERMITTED_DATA_INCREASE * 2
    )


def create_same_accounts_internal(
    user_account: Caller,
    second_account: Caller,
    evm_loader: EvmLoader,
    operator_keypair: Keypair,
    treasury_pool: TreasuryPool,
    iterations: int,
    ...
    deposit_neon(evm_loader, operator_keypair, user_account.eth_address, ONE_TOKEN)
    deposit_neon(evm_loader, operator_keypair, second_account.eth_address, ONE_TOKEN)

    contract = create_contract_address(user_account, evm_loader)
    holder_acc = create_holder(operator_keypair)
    deployment_tx = make_deployment_transaction(user_account, BIG_CONTRACT_FILENAME)
    ...
    try:
        ...
        check_account_initialized_in_another_trx_exception(e, contract.solana_address)
...
def check_account_initialized_in_another_trx_exception(exception: RPCException, solana_address: PublicKey):
    ...
    for log in error['data']['logs']:
        if f'Blocked nonexistent account {solana_address} was created/initialized outside current transaction' in log:
            return
