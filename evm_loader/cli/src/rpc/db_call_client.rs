...
use std::any::Any;
use super::{Rpc, e};
use crate::{types::{TracerDb, DbConfig, TxParams}};
...
impl Rpc for CallDbClient {
    ...
    fn get_account(&self, key: &Pubkey) -> ClientResult<Account>  {
       self.tracer_db.get_account_at(key, self.slot)
            .map_err(|e| e!("load account error", key, e))?
            .ok_or_else(|| e!("account not found", key))
    }

    fn get_account_with_commitment(&self, key: &Pubkey, _: CommitmentConfig) -> RpcResult<Option<Account>> {
    ...
    fn get_block(&self, slot: Slot) -> ClientResult<EncodedConfirmedBlock>{
        let hash = self.tracer_db.get_block_hash(slot).map_err(|e| e!("get_block error", slot, e))?;
    ...
    fn get_block_time(&self, slot: Slot) -> ClientResult<UnixTimestamp>{
        self.tracer_db.get_block_time(slot).map_err(|e| e!("get_block_time error", slot, e))
    }
