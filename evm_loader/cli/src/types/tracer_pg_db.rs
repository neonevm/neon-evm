use {
    ...
    super::{do_connect, DbConfig, block, PgError, PgResult},
...
impl TracerDb {
    ...
    pub fn get_block_hash(&self, slot: u64) -> PgResult<String>{
        let slot: i32 = slot.try_into().map_err(|e| PgError::Custom(format!("slot cast error: {e}")))?;

        let row = block(|| async {
            self.client.query_one(
                "SELECT blockhash FROM public.block WHERE slot = $1", &[&slot],
            ).await
        })?;
    ...
    pub fn get_block_time(&self, slot: u64) -> PgResult<i64> {
        ...
        let row = block(|| async {
            self.client.query_one(
                "SELECT block_time FROM public.block WHERE slot = $1", &[&slot],
            ).await
        })?;
    ...
    pub fn get_account_at(&self, pubkey: &Pubkey, slot: u64) -> PgResult<Option<Account>> {
        ...
        Ok(Some(Account {
            lamports: u64::try_from(lamports).map_err(|e| PgError::Custom(format!("lamports cast error: {e}")))?,
            data: row.try_get(5)?,
            owner: Pubkey::try_from(owner).map_err(|e| PgError::Custom(format!("owner cast error: {e}")))?,
            executable: row.try_get(3)?,
            rent_epoch: u64::try_from(rent_epoch).map_err(|e| PgError::Custom(format!("rent_epoch cast error: {e}")))?,
        }))
    }

    pub fn get_account_by_sol_sig(&self, pubkey: &Pubkey, sol_sig: &[u8; 64]) -> PgResult<Option<Account>> {
        let pubkey_bytes = pubkey.to_bytes();
        let row = block(|| async {
            self.client.query_one(
