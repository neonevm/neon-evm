...
impl TracerDb {
    ...
    pub fn get_block_hash(&self, slot: u64) -> PgResult<String> {
        ...
        let row = block(|| async {
            self.client
                .query_one(
                    "SELECT blockhash FROM neon_history.block WHERE slot = $1",
                    &[&slot],
                )
                .await
        })?;
    ...
    pub fn get_block_time(&self, slot: u64) -> PgResult<i64> {
        ...
        let row = block(|| async {
            self.client
                .query_one(
                    "SELECT block_time FROM neon_history.block WHERE slot = $1",
                    &[&slot],
                )
                .await
        })?;
    ...
    pub fn get_latest_block(&self) -> PgResult<u64> {
        let row = block(|| async {
            self.client
                .query_one("SELECT MAX(slot) FROM neon_history.slot", &[])
                .await
        })?;
