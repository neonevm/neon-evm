...
class EthereumTransaction:
    ...
    def __init__(self, ether_caller, contract_account, contract_code_account):
        self.ether_caller = ether_caller
        self.contract_account = contract_account
        self.contract_code_account = contract_code_account
        self.solana_ether_caller = None  # is created in NeonEvmClient.create_instruction_data_from_tx
        self.trx_data = None  # is created in prepare
        self.trx_account_metas = None  # is created in prepare
        self.storage = None  # is created in NeonEvmClient.send_emv_transaction
        self.iterative_steps = 100

    def prepare(self, trx_data, trx_account_metas=None):
...
class NeonEvmClient:
    ...
    def create_solana_ether_caller(self, ethereum_transaction):
        ...
        print("Solana ether caller account:", ethereum_transaction.solana_ether_caller)
    ...
    def create_instruction_data_from_tx(self, ethereum_transaction):
        self.create_solana_ether_caller(ethereum_transaction)
        caller_trx_cnt = getTransactionCount(client, ethereum_transaction.solana_ether_caller)
    ...
    def create_trx(self, ethereum_transaction, keccak_data, data):
    ...
    def send_neon_transaction(self, evm_trx_data, ethereum_transaction, need_storage=False) -> types.RPCResponse:
        (from_address, sign, msg) = self.create_instruction_data_from_tx(ethereum_transaction)
        keccak_data = make_keccak_instruction_data(1, len(msg), 9 if need_storage else 1)
        data = evm_trx_data + from_address + sign + msg
        trx = self.create_trx(ethereum_transaction, keccak_data, data)
        if need_storage:
            if ethereum_transaction.storage is None:
                ethereum_transaction.storage = self.create_storage_account(sign[:8].hex())
            trx.instructions[-1].keys \
                .insert(0, AccountMeta(pubkey=ethereum_transaction.storage, is_signer=False, is_writable=True))
    ...
    def send_ethereum_trx_iterative(self, ethereum_transaction) -> types.RPCResponse:
        ...
        while True:
            ...
            if result['result']['meta']['innerInstructions'] \
                ...
                if data[0] == 6:
                    ethereum_transaction.storage = None
                    return result

    def send_ethereum_trx_single(self, ethereum_transaction) -> types.RPCResponse:
        assert (isinstance(ethereum_transaction, EthereumTransaction))
        return self.send_neon_transaction(bytes.fromhex("05"), ethereum_transaction)
