...
class EventTest(unittest.TestCase):
    ...
    def sol_instr_10_continue_with_holder(self, storage_account, step_count, contract, code):
        return TransactionInstruction(
            program_id=self.loader.loader_id,
            data=bytearray.fromhex("0A") + step_count.to_bytes(8, byteorder='little'),
            keys=[
    ...
    def call_with_holder_account(self, input, contract, code):
        ...
        while (True):
            print("Continue")
            trx = Transaction()
            trx.add(self.sol_instr_10_continue_with_holder(storage, 400, contract, code))
            result = send_transaction(http_client, trx, self.acc)["result"]
