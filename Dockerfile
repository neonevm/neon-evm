...
COPY evm_loader/test_requirements.txt solana-py.api.patch solana-py.commitment.patch /tmp/
RUN pip3 install -r /tmp/test_requirements.txt
RUN cd /usr/local/lib/python3.8/dist-packages/ && patch -p0 </tmp/solana-py.api.patch && patch -p0 </tmp/solana-py.commitment.patch
